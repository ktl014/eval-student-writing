...
class TestModel():
    ...
    def data_module(self):
        data_module = hydra.utils.instantiate(cfg.data.datamodule, _recursive_=False)
    ...
    def test_unit(self, model):
        ...
        assert type(model.recall_macro_metric) == torchmetrics.Recall(
            average="macro", num_classes=num_labels
        )
        assert type(model.precision_micro_metric) == torchmetrics.Precision(average="micro")
        assert type(model.recall_micro_metric) == torchmetrics.Recall(average="micro")
    ...
    def test_foward(self, data_module, model):
        ...
        #=== Trigger Output ===#
        outputs = model.forward(batch['input_ids'], batch['attention_mask'], batch[gc.LABEL])
